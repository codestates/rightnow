...
import { userIsLogin, userNickname, userProfile, userRole } from '../../reducers/userSlice';
import { Link } from 'react-router-dom';
import { mypageMenu, showModal, showMypage } from '../../reducers/componetSlice';
...
const Header = () => {
  ...
  return (
    <>
      <Container className="bg-gray-50 h-16 flex shadow-md justify-center fixed w-full z-40 top-0">
        <Content className="flex w-7/12 justify-between">
          <Link to={isLogin ? '/search' : '/'} className="flex items-center">
            <div className="flex items-center space-x-2 cursor-pointer">
              <Logo />
              <p className=" text-sub font-bold text-4xl whitespace-nowrap w-auto ">Rightnow</p>
            </div>
          </Link>
          <div className="flex items-center relative w-72 justify-end pr-8 z-30">
            {!isLogin && (
              <Link to="/auth/login" className="flex items-center">
                <button className="rounded-lg whitespace-nowrap bg-main group relative" style={{ width: 86, height: 43 }}>
                  <div
                    className="h-full rounded-lg bg-gray-50 w-4 opacity-0 relative -left-1 group-hover:w-24 group-hover:opacity-100"
                    style={{ transition: '0.3s' }}
                  />
                  <span className=" absolute top-3 left-4.5 font-semibold text-gray-50 transition-all group-hover:text-main">Sign in</span>
                </button>
              </Link>
            )}
            {isLogin && (
              <div className=" text-slate-600 font-semibold text-sm mr-2 whitespace-nowrap text-ellipsis">
                {role === 'ADMIN' ? `${nickname}(관리자)님 환영합니다` : `${nickname}님 환영합니다`}
              </div>
            )}
            {isLogin && (
              <div
                ...
                <div
                  ...
                  style={{
                    backgroundImage: `url(${
                      profile === null
                        ? defaultProfile
                        : profile.indexOf('kakaocdn') !== -1 || profile.indexOf('googleusercontent') !== -1
                        ? profile
                  ...
                  if (id === 'user') {
                    return (
                      <>
                        ...
                        >
                          <p className="hover:bg-gray-100 cursor-pointer px-2 py-2">{label}</p>
                        </Link>
                      </>
                    );
                  } else {
