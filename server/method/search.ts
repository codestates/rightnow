  ...
  try {
    ...
    */
    socket.on('searching_check', async (data: any): Promise<void> => {
      ...
      if (find) {
        ...
        searchNamespace
          .to(socket.id)
          .emit('searching_check', { message: 'ok' });
      }
    });
    /*
    ...
    */
    ...
    socket.on('find_room', async (data: CacheUser | any): Promise<void> => {
      ...
      if (
        canEnter.message === 'exist' ||
        canEnter.message === 'someone exist'
      ) {
        //클라이언트에서 해당 룸으로 리다이렉션 시킴
        if (
          data.type === 'ALONE' ||
      ...
      let findRoom: any = tempRooms.find((item: any) => {
        let allowEnter: boolean =
          data.type === 'ALONE'
            ...
            : item.allow_num >
              item.participants.length + data.email_list.length;
        return (
      ...
      try {
        ...
        if (findUser.type === 'GROUP') {
          for (let email of data.email_list) {
            ...
            searchNamespace
              .to(email)
              .emit('cancel', { is_master: false, uuid });
          }
        }
        findUsers.delete(data.email);
